...
- 本文件**不含任何真实仓库地址或凭证**：`<URL>` 由仓库所有者本人替换。
- **本清单尚未执行**：仓库当前**没有配置远端**，也**没有 push 过任何内容**。

> ### 执行主体声明（重要）
>
> 本清单中的所有**远端写操作**（`git remote add`、`git push`）**一律由仓库所有者本人执行**。
> **AI 不接触凭证**：AI 不读取、不转写、不代填、不传递任何 API key / token / 密码，
> 也不代替所有者执行 push；需要凭证的每一步都由所有者亲手完成。
> AI 的职责止于**在本地准备好可复现的提交与这份清单**。
...
## 2. 添加远端并首次推送（**由仓库所有者本人执行；AI 不接触凭证、不代执行 push**）
...
- 本节命令**请勿交给 AI 代执行**：所有者本人登录自己的账号执行即可；CI/脚本中不要嵌入任何凭证。
