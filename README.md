...
浏览器 computer-use 的**决策骨架**：Jev（TypeSafe System One）做每步快判断，LLM 仅在低置信时兜底。

- 每步一次 HTTP 三问：选哪个元素（choice）/ 有多确信（score）/ 是否完成（noul）——约 1 s、约 900 tokens。
- 仅当 **margin < 0.30** 时升级 LLM；需要生成文本的场合（填表内容）由 LLM 负责。
- 只以 **MCP（stdio）** 与 **SKILL.md** 对外暴露，代码无宿主分支——不绑定任何宿主。
- **仓库不含任何 API key**，由使用者自备；实测单步 87.5%、多步 93.3%（详见 `eval/`）。

## 2. 部署的主要注意事项

- **环境**：Python ≥ 3.9（MCP 需 ≥ 3.10）+ `playwright` + `typer`；需 `playwright install chromium`。
- **凭证**：`cp .env.example .env` 后自填 `JEV_API_KEY`（申请：<https://console.typesafe.ai>）；`LLM_*` **可选**，不填则低 margin 步沿用 Jev 判断。
- **`JEV_BASE_URL` 必须是** `https://api.typesafe.ai`（不是 `console.*`，指错会 307 → `/login`）。
- **先体检**：`jev-cu doctor`（退出码 `0/2/3/4/5/6` = 正常/缺 key/base 错/鉴权失败/服务不可用/网络不可达）。
- **阈值**：`JEV_ROUTE_T=0.30`、`JEV_DONE_T=0.50`；`.env` 已在 `.gitignore`，**勿提交**。
