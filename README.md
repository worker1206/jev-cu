...
把 **Jev（TypeSafe System One）** 用作「系统一」快判断层的浏览器 computer-use 骨架：每个动作步只发一次请求，同时问三件事——选哪个元素、有多确信、是否完成；仅当 `margin < 0.30` 时升级到「系统二」（OpenAI 兼容 LLM）做判断，需要生成的文本（要填进输入框的内容）也由它负责。

能力只通过 **MCP（stdio）** 与 **SKILL.md** 对外暴露，代码里没有宿主分支，**不绑定任何宿主**。仓库不含任何 API key，使用前请按下方「安装」「用法」自行配置。真实站点实测：单步决策 87.5%、多步链路 93.3%（明细见 `eval/`）。
